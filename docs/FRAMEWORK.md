...
`risk_framework.collection.load_collection()` consumes the `risk_collector` report contract documented in [DATA_COLLECTION.md](DATA_COLLECTION.md). `WebsiteDataset` joins those reports to the existing scoreboard contract inside the [website payload](WEBSITE_DATA.md). Exporting reads local files only; it never invokes the collector pipeline.

A collected report is a dated evidence snapshot. Its category score is a capped, severity/confidence-weighted heuristic produced upstream, not an AI ensemble. The adapter preserves this value without reconstructing the scoring formula. It validates scores, summary counts against sourced signals, requested category sets, source-check statuses, and report identity. Errors reject the input batch instead of silently dropping reports.
